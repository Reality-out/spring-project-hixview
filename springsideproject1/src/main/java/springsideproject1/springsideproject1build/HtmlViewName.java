package springsideproject1.springsideproject1build;

public class HtmlViewName {
    /**
     * USER
     */
    public static String USER_CREATE_MEMBERSHIP = "user/membership/";
    public static String MANAGER_REMOVE_MEMBERSHIP = "manager/remove/membership/";

    /**
     * ARTICLE - COMPANY
     */
    public static String MANAGER_ADD_COMPANY_ARTICLE = "manager/add/article/company/";
    public static String MANAGER_REMOVE_COMPANY_ARTICLE = "manager/remove/article/company/";
}
