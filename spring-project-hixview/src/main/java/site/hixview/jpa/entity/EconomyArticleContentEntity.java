...
public class EconomyArticleContentEntity {
    ...
    @ManyToOne(fetch = FetchType.LAZY, cascade = {CascadeType.PERSIST, CascadeType.MERGE}, optional = false)
    @JoinColumn(name = ECON_CONT_NUM_SNAKE, foreignKey = @ForeignKey(value = ConstraintMode.NO_CONSTRAINT))
    ...
    public EconomyArticleContentEntity(Long number, EconomyArticleEntity economyArticle, EconomyContentEntity economyContent) {
        this.number = number;
