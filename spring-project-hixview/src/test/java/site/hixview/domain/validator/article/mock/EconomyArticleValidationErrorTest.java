...
class EconomyArticleValidationErrorTest implements EconomyArticleTestUtils {
    ...
    void futureDateEconomyArticleAdd() throws Exception {
        ...
        // then
        Map<String, Object> jsonMap = new ObjectMapper().readValue(mockMvc.perform(postWithEconomyArticleDto(ADD_SINGLE_ECONOMY_ARTICLE_PATH, articleDtoFuture))
    ...
    void duplicatedNameEconomyArticleAdd() throws Exception {
        ...
        // then
        Map<String, Object> jsonMap = new ObjectMapper().readValue(mockMvc.perform(postWithEconomyArticleDto(ADD_SINGLE_ECONOMY_ARTICLE_PATH, articleDtoName))
                .andExpectAll(status().isBadRequest()).andReturn().getResponse().getContentAsString(), new TypeReference<>(){});
        assertThat(jsonMap.get(IS_BEAN_VALIDATION_ERROR)).isEqualTo(false);
    ...
    void duplicatedLinkEconomyArticleAdd() throws Exception {
        ...
        // then
        Map<String, Object> jsonMap = new ObjectMapper().readValue(mockMvc.perform(postWithEconomyArticleDto(ADD_SINGLE_ECONOMY_ARTICLE_PATH, articleDtoLink))
                .andExpectAll(status().isBadRequest()).andReturn().getResponse().getContentAsString(), new TypeReference<>(){});
        assertThat(jsonMap.get(IS_BEAN_VALIDATION_ERROR)).isEqualTo(false);
