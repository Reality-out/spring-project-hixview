...
public class MockValidatorBeanFactoryPostProcessor implements BeanFactoryPostProcessor {
    ...
    public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) {
        ...
        for (BeanDefinition validatorDef : scanner.findCandidateComponents(VALIDATOR_BASE_PACKAGE)) {
            Class<?> clazz;
            try {
                clazz = ClassUtils.forName(Objects.requireNonNull(validatorDef.getBeanClassName()), classLoader);
            } catch (ClassNotFoundException e) {
                throw new RuntimeException("해당 검증자 클래스를 불러오는 데 실패하였습니다: " + validatorDef.getBeanClassName());
            }
