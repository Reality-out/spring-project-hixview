...
public class MemberRepositoryJdbc implements MemberRepository {
    ...
    public Optional<Member> findMemberByIdentifier(Long identifier) {
        return Optional.ofNullable(jdbcTemplate.query("select * from members where identifier = ?", memberRowMapper(), identifier).getFirst());
    }

    @Override
    public Optional<Member> findMemberByID(String id) {
        List<Member> oneMemberOrNull = jdbcTemplate.query("select * from members where id = ?", memberRowMapper(), id);
        return oneMemberOrNull.isEmpty() ? Optional.empty() : Optional.of(oneMemberOrNull.getFirst());
    }

    @Override
    public List<Member> findMemberByName(String name) {
        return jdbcTemplate.query("select * from members where name = ?", memberRowMapper(), name);
    }
    ...
    public void saveMember(Member member) {
        ...
        jdbcInsert.withTableName("members").usingGeneratedKeyColumns("identifier");
    ...
    public void removeMemberByID(String id) {
        jdbcTemplate.execute("delete from members where id = '" + id + "'");
    }
